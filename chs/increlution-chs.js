...
var cnItems = {
    ...
    "Cooks 1 fish per completion (": "每次完成烹饪 1 条鱼（",
    "), requires a raw fish to cook": "），需要生鱼来烹饪",
    "Fishing skill": "钓鱼技巧",
    ...
    "It appears that the camp was from the same tribe that you've met on the beach. But all they left behind was an empty pan": "营地似乎来自您在海滩上遇到的同一个部落，但他们留下的只是一个空锅",
    ...
    "Shield claimed. Now that you're able to protect yourself a bit better in battle, you feel slightly more comfortable heading into the next fight": "得到盾牌。既然你能够在战斗中更好地保​​护自己，那么进入下一场战斗你会感觉更舒服一些",
    ...
    "Bakes 1 pie per completion (": "每次完成烘烤 1 个馅饼（",
    "), requires dough to bake": "），需要面团烘烤",
    ...
    "Cooks 1 piece of bull meat per completion (": "每次完成烹饪 1 块牛肉（",
    "), requires bull meat to cook": "），需要牛肉来烹饪",
    "Cooks 1 shark per completion (+59.5e3 health when eaten), requires a raw shark to cook": "每次完成烹饪 1 条鲨鱼（食用时 +59.5e3 生命值），需要生鲨鱼来烹饪",
    "Cooks 1 shark per completion (": "每次完成烹饪 1 条鲨鱼（",
    "), requires a raw shark to cook": "），需要生鲨鱼来烹饪",
    "Expands the": "展开",
    ...
    "Cooks 1 goat per completion (": "每次完成烹饪 1 只山羊（",
    "), requires a dead goat to cook": "），需要一只死山羊来烹饪",
    "Cooks 1 rabbit per completion (+71.4 health when eaten), requires a dead rabbit to cook": "每次完成烹饪 1 只兔子（食用时 +71.4 生命值），需要一只死兔子来烹饪",
    "Cooks 1 rabbit per completion (": "每次完成烹饪 1 只兔子（",
    "), requires a dead rabbit to cook": "），需要一只死兔子来烹饪",
    "Cooks 1 seal per completion (+297e3 health when eaten), requires a raw seal to cook": "每次完成烹饪 1 个海豹（食用时 +297e3 生命值），需要生海豹才能烹饪",
    "Cooks 1 seal per completion (": "每次完成烹饪 1 个海豹（",
    "), requires a raw seal to cook": "），需要生海豹才能烹饪",
    ...
    "Cooks 1 vulture per completion (": "每次完成烹饪 1 只秃鹰（",
    "), requires a raw vulture to cook": "）需要生秃鹰来烹饪",
    ...
    "These beasts may look harmless, but they definitely aren't. Every completion causes ": "这些野兽可能看起来无害，但绝对不是。每次完成都会造成 ",
    " damage, and provides 1 raw seal per completion (Can be cooked": " 伤害，并且每次完成提供 1 个生海豹（可以煮熟",
    ...
    "Very hot. Every completion causes ": "很热。每次完成造成 ",
    " damage, and provides 1 magma block per completion (Used for construction": " 伤害，并且每次完成提供 1 个熔岩块（用于建造",
    ...
    "Bakes 1 biscuit per completion (": "每次完成烘烤 1 个饼干（",
    "), requires dough to bake": "），需要面团烘烤",
    ...
    "Gives you 1 portion of stew per completion (": "每次完成给你一份炖肉（",
    "). It's gray and has some bean-like things in it. Fishing out the parts that seem somewhat edible takes some effort. Best eaten with nose closed": "）。它是灰色的，里面有一些类似豆子的东西。捞出看起来有些可食用的部分需要一些努力。最好闭着鼻子吃",
    "health decay": "生命值衰退",
    ...
    "Cooks 1 portion of soup per completion (+119e3 health when drunk). You already put all the ingredients in the pot! Only available in camp": "每次完成煮 1 份汤（喝下时 +119e3 生命值）。你已经把所有的原料都放进锅里了！仅在营地可用",
    ...
    "Increases the amount of maximum health you gain after life by 10% per perk tier": "每级额外增加转生时 10% 的生命值上限",
    ...
    "Provides 1 herring per completion (": "每次完成提供 1 条鲱鱼（",
    ...
    "Provides 1 orange per completion (": "每次完成提供 1 个橙子（",
    "Run first, ask questions later. Let's get out of here!": "先跑，后问。 让我们离开这里！",
    ...
    "Bakes 1 bread per completion (": "每次完成烘焙 1 个面包（",
    "), requires a grain to bake": "），需要谷物来烘焙",
    "Bakes 1 cake per completion (+150 health when eaten), requires a grain to bake": "每次完成烘焙 1 个蛋糕（食用时 +150 生命值），需要谷物来烘焙",
    "Bakes 1 cake per completion (": "每次完成烘焙 1 个蛋糕（",
    "), requires a grain to bake": "），需要谷物来烘焙",
    ...
    "Cooks 1 boar per completion (": "每次完成烹饪 1 头野猪（",
    "), requires a dead boar to cook": "），需要一头死猪来烹饪",
    ...
    "Cooks 1 crab per completion (": "每次完成烹饪 1 只螃蟹（",
    "), requires a raw crab to cook": "），需要生螃蟹来烹饪",
    ...
    "Shake tree, catch what falls out! Provides 1 coconut per completion (": "摇树，抓住掉出来的东西！ 每次完成提供 1 个椰子（",
    ...
    "Provides 1 banana per completion (": "每次完成提供 1 个香蕉（",
    ...
    "Cooks 1 lobster per completion (": "每次完成烹饪 1 只龙虾（",
    "), requires a raw lobster to cook": "），需要生龙虾来烹饪",
    ...
    "Cooks 1 pelican per completion (": "每次完成烹饪 1 只鹈鹕（",
    "), requires a dead pelican to cook. Tastes surprisingly close to fish, for a bird": "），需要一只死鹈鹕来烹饪。 对鸟来说，味道惊人地接近鱼",
    ...
    "Cooks 1 eel per completion (": "每次完成烹饪 1 条鳗鱼（",
    "), requires a raw eel to cook": "），需要生鳗鱼来烹饪",
    ...
    "Provides 1 apple per completion (": "每次完成提供 1 个苹果（",
    ...
    "Provides 1 moss per completion (": "每次完成提供 1 个苔藓（",
    "), full of vitamins and .. flavors": "），富含维生素和 .. 风味",
    "Provides 1 tomato per completion (+2.00e4 health when eaten": "每次完成提供 1 个番茄（食用时 +2.00e4 生命值",
    "Provides 1 tomato per completion (": "每次完成提供 1 个番茄（",
    ...
    "Cooks 1 falcon per completion (": "每次完成烹饪 1 只猎鹰（",
    "), requires a dead falcon to cook": "），需要一只死猎鹰来烹饪",
    ...
    "Cooks 1 portion of soup per completion (": "每次完成煮 1 份汤（",
    "). You already put all the ingredients in the pot! Only available in camp": "）。 你已经把所有的原料都放进锅里了！ 仅在营地可用",
    ...
    "Shovel coal": "铲煤",
    "Sell food": "出售食物",
    "Buy mixed salad": "购买什锦沙拉",
    "Buy packed meal": "购买盒饭",
    "Gather steel": "收集钢材",
    "Gather clam": "采集蛤蜊",
    "Cook clam": "烹饪蛤蜊",
    "Cut moldy wood": "切割发霉的木材",
    "Dig titan stone": "挖泰坦石",
    "Shoot beehive": "射击蜂巢",
    "Pick portobello": "选择portobello",
    "Ceremonial fire": "仪式火",
    ...
    "Cross clouds": "穿越云层",
    "Land on cliff": "着陆悬崖",
    "Meet cliff people": "认识悬崖上的人",
    "Continue cliff chat": "继续悬崖聊天",
    "Enter mountain": "进入山区",
    "Climb to low layer": "爬到低层",
    "Enter forage shack": "进入饲料窝棚",
    "Meet forager": "认识觅食者",
    "Mushroom school": "蘑菇学校",
    "Exit shack": "离开窝棚",
    "Walk to waterfall": "走到瀑布",
    "Meet cleaner": "认识清洁工",
    "Catch soles": "抓鞋底",
    "Catch cart": "抓车",
    "Catch statue": "抓雕像",
    "Climb back up": "重新爬上去",
    "Enter main square": "进入主广场",
    "Introduce yourself": "自我介绍",
    "Ignite the fire": "点燃火",
    "Enter the crowd": "进入人群",
    "Exit the square": "离开广场",
    "Walk to top layer": "走到顶层",
    "Meet blacksmith": "认识铁匠",
    "Approach dog": "接近狗子",
    "Take dog's toy": "拿起狗子的玩具",
    "Follow dog": "跟着狗子",
    "Pillage nest": "掠夺巢穴",
    "Shoo crows": "嘘赶乌鸦",
    "Empty nest": "空巢",
    ...
    "Talk to keepers": "与饲养员交谈",
    "Enter the vault": "进入金库",
    "Approach statue": "走近雕像",
    "Attack titan": "攻击泰坦",
    "Dodge titan": "嘲讽泰坦",
    "Backshot titan": "后射泰坦",
    "Trip titan": "绊倒泰坦",
    "Block titan": "格挡泰坦",
    "Counter titan": "反击泰坦",
    "Approach debris": "接近碎片",
    "Fire at titan": "向泰坦开火",
    "Fire at ceiling": "向天花板开火",
    "Return to titan": "返回泰坦",
    "Read inscription": "阅读铭文",
    "Inspect core": "检查核心",
    "Funnel titan core": "漏斗泰坦核心",
    "Disable titan core": "禁用泰坦核心",
    "Exit the vault": "离开金库",
    "Enemy": "敌人",
    "Titan arrow": "泰坦之箭",
    "Falling debris": "掉落的碎片",
    ...
    "Moldy wood": "发霉的木头",
    "Titan stone": "泰坦石",
    "There's a lot of commotion in hightown. It seem like today is their annual sports competition, with some cool prizes that you can cash for tools. It appears that the prizes are silver coins, of which you already have some! Let's see what the vendor has to offer": "上城闹得沸沸扬扬。 好像今天是他们一年一度的体育比赛，有一些很酷的奖品，您可以用这些奖品换取工具。 看来奖品是银币，你已经有一些了！ 让我们看看供应商必须提供什么",
    "After a little staring in disbelief, one of the people on the cliff starts talking: \"A dragon rider?! Those were supposed to be legends\". But before continuing they notice that you're looking fairly hungry, and point you towards their koi pond. It looks like they have a bit of an overpopulation problem in the pond, so seem to be totally ok with you catching some": "难以置信地瞪了一眼后，悬崖上的一人开口说道：“龙骑士？！那应该是传说。” 但在继续之前，他们会注意到您看起来很饿，然后将您指向他们的锦鲤池。 看起来他们在池塘里有点人口过剩的问题，所以你抓一些似乎完全没问题",
    "After walking through the entrance on the cliff, you can't help but feel surprised by how many people actually live inside this mountain. It must be housing hundreds. Luckily for you, there's koi ponds everywhere and the overpopulation in those ponds is notable. That said, there's stairs everywhere, and one enormous fortified vault in the middle of the city. It appears that you're welcome to travel through the city on your own": "走过悬崖上的入口，不禁感叹这座山里面居然住着多少人。 它必须容纳数百人。 幸运的是，到处都有锦鲤池塘，而且这些池塘里的鱼群非常多。 也就是说，到处都是楼梯，在城市中央还有一个巨大的坚固金库。 看来欢迎你一个人穿城而过",
    "It looks like people may have passed through here, but those monkeys seem more interested in the food in your inventory than anything else": "看起来有人可能经过这里，但那些猴子似乎对你库存中的食物比其他任何东西都更感兴趣",
    "Now that you've passed through the most dense parts of the clouds, you have a fairly clear view of where the dragon is bringing you. It looks as if a whole civilization has taken residence within a mountain. The construction appears to use the mountain itself as a foundation, whereas both smoke and light exit through various what appear to be windows. Getting closer, one cliff seems to stick out a fair bit. It looks like the dragon is aiming to land there": "现在你已经穿过了云层最稠密的部分，你对巨龙要带你去哪里有了相当清晰的认识。 整个文明似乎都栖息在一座山中。 该建筑似乎以山本身为基础，而烟雾和光线都通过各种看似窗户的东西排出。 越来越近，一个悬崖似乎有点突出。 看起来这条龙正打算降落在那里",
    "One of the people on the cliff feels like a proper introduction is appropriate. \"We're the guardians of the vault. We've been living in this mountain for most of our lives, dedicated to protect what's inside the vault. We can't exactly say what we're guarding, but legend tells us it's defended by something created by someone more than mortal. In any case, you're welcome to enter our city, dragon rider\"": "悬崖上的一个人觉得适当介绍一下是合适的。 “我们是金库的守护者，我们大半辈子都生活在这座山上，致力于保护金库中的东西。我们不能确切地说出我们在守护什么，但传说告诉我们它被保护了 由非凡人创造的东西。无论如何，欢迎你进入我们的城市，龙骑士”",
    "That's what we call a leap of faith! But you've actually managed to make it across the gorge in one jump! You hear the sound of monkeys in the distance. It's probably a good idea to check on that commotion": "这就是我们所说的信仰之跃！ 但你实际上已经成功地一次跳跃穿过了峡谷！ 你听到远处传来猴子的叫声。 检查一下骚动可能是个好主意",
    "This beast is fast, but you've managed to hold on so far! It seems very determined in where it's bringing you. Almost as if it's done this before? In the distance, you see an enormous construction peeking through the clouds. And the dragon proceeds to fly directly to it": "这只野兽速度很快，但你已经坚持到现在了！ 它似乎很确定它会把你带到哪里。 几乎就像它以前做过的一样？ 在远处，您会看到一个巨大的建筑从云层中窥视。 龙直接飞向它",

    "Gorilla, beaten. That'll show them not to mess with you! The other monkeys quickly flee the scene. No food for them today!": "大猩猩, 击败. 那会让他们知道不要惹你！ 其他猴子迅速逃离现场。 今天他们没有食物！",
    ...
    "Shield rejected. Whether or not that was a good decision is unclear, but the crowd definitely likes it!": "拒绝盾牌。 这是否是一个好的决定尚不清楚，但人群肯定喜欢它！",
    ...
    "The crowd actually became a little quiet, stunned by disbelief of how you've managed to defeat their strongest beast. That was believed to be impossible. At this point you're starting to scare them, and the announcer asks you to leave the arena with a tremble in the voice": "人群竟然变得有些安静，目瞪口呆，不敢相信你竟然打败了他们最强的野兽。 这被认为是不可能的。 这时候你开始吓唬他们了，播音员声音颤抖的叫你离开赛场",
    "The crowd actually became a little quiet, stunned by disbelief of how you've managed to defeat their strongest beast. That was believed to be impossible. At this point you're starting to scare them, and the announcer asks you to leave the arena with a tremble in the voice": "哇！ 你居然在宝库中打败了巨龙！ 我想它可以再次有人居住。 虽然住在这里的人现在可能早已不在了",
    ...
    "Expand the shrine for another": "扩建神社来获得另外",
    "own 25 eggs": "拥有25枚鸡蛋",
    "(finish chep6)": "(完成章节6)",
    "(finish chep7)": "(完成章节7)",
    "(finish chep8)": "(完成章节8)",
	...
	"This beast is fast, but you've managed to hold on so far! It seems very determined in where it's bringing you. Almost as if it's done this before? In the distance, you see an enormous construction peeking through the clouds. And the dragon proceeds to fly directly to it": "这只野兽速度很快，但你已经坚持到现在了！ 它似乎很确定它会把你带到哪里。 几乎就像它以前做过的一样？ 在远处，您会看到一个巨大的建筑从云层中窥视。 龙直接飞向它",
    "Well that wasn't all that bad. The dragon safely landed on the cliff and you're able to jump right off its back! Bringing you here seems to have been its entire purpose, and it flies away almost immediately. After taking a quick moment to come to your senses, you notice that a couple of odd-looking people are staring at you": "好吧，这还不算太糟糕。 龙安全地降落在悬崖上，你可以从它的背上跳下来！ 带你来这里似乎就是它的全部目的，几乎是立刻就飞走了。 迅速回过神来后，您注意到几个长相古怪的人正盯着您看",
    "Approach the vault in the center of this city. A massive door that's easily six times your size and visible from almost anywhere in town. But it's common courtesy to visit the other parts of town first!": "接近这座城市中心的金库。 一扇巨大的门，大小是你的六倍，几乎在城里的任何地方都能看到。 但首先参观城镇的其他地方是常见的礼貌！",
    "Colorful and cookable": "色彩缤纷，可烹饪",
    ...
    "It's quite a few stairs down, but nothing else is stopping you from going to the lower layer of the city": "下了好几层楼，但没有什么能阻止你去城市的下层",
    "This area is entirely paved, so you won't be able to gather local food sources in the main square": "该区域完全铺设，因此您无法在主广场收集当地食物来源",
    "You hear commotion coming from the center area of the city.": "你听到来自城市中心区域的骚动。",
    "You see some smoke coming from the higher layer of the city": "你看到一些烟从城市的高层冒出来",
    ...
    "A little shack that looks open. It has a little sign outside that appears to portray a mushroom": "一个看起来敞开的小棚屋。 它外面有一个小标志，似乎描绘了一个蘑菇",
    ...
    "Contains a wooden bow and a copper shovel, which increase the skill multipliers of": "包含一把木弓和一把铜铲，可增加",
    "Cooks 1 koi carp per completion (+5.59e5 health when eaten), requires a raw koi carp to cook": "每次完成烹饪 1 条锦鲤（食用时 +5.59e5 生命值），需要一条生锦鲤来烹饪",
    "Cooks 1 koi carp per completion (": "每次完成烹饪 1 条锦鲤（",
    "), requires a raw koi carp to cook": "），需要一条生锦鲤来烹饪",
    "digging and": "挖掘和",
    "Every completion causes 5.00e5 damage, and provides 1 steel per completion (Used for construction": "每次完成造成 5.00e5 伤害，每次完成提供 1 钢（用于建筑",
    "Every completion causes": "每次完成造成",
    "damage, and provides 1 steel per completion (Used for construction": "伤害，每次完成提供 1 钢（用于建筑",
    "Farming skill multiplier of 5.00e9 to succeed, and fails if your multiplier is lower": "农业技能乘数 5.00e9 成功，如果你的乘数较低则失败",
    "Hardly comfortable, but very robust. Lowers": "不太舒服，但非常坚固。 降低",
    "hunting by 25%. These are the same tools as were available in the chapter 5 city, and do not stack.": "狩猎 25%。 这些工具与第 5 章城市中可用的工具相同，并且不叠加。",
    "Ignite the fire to ceremonially start the festivities!": "点燃圣火，隆重开始庆祝活动！",
    "Increases the health value of all your food by 10% for the rest of this generation, without any penalty": "为这一代的其他人增加 10% 的所有食物的健康值，没有任何惩罚",
    "Increases the multiplier of all your skills by 10": "将你所有技能的乘数提高 10",
    "Leave the forager's shack and continue exploring the lower layer of the city": "离开觅食者的小屋，继续探索城市的下层",
    "Leave the lower layer and return to the entrance of the city": "离开下层回到城口",
    "maximum health, but replaces your iron belt if you have one (you don't": "最大生命值，但替换你的铁腰带（你没有",
    "maximum health, but replaces your iron belt if you have one (you do, effectively making this a 33.3% bonus": "最大生命值，但替换你的铁腰带（你有，这使奖励效果为33.3%",
    "Mingle with the people in the main square to find out what's going on": "与主广场上的人们打成一片，了解发生了什么",
    "Place coal under the ceremonial fire, so it can be ignited": "将煤放在礼火下，这样它就可以被点燃",
    "Proceed to fly through the clouds, towards the construction that the dragon seems to bring you to": "继续飞过云层，飞向龙似乎带你去的建筑",
    "Proceed to meet the people on the cliff": "继续去见悬崖上的人",
    "Provides 1 raw koi carp per completion (Colorful and cookable": "每次完成提供 1 条生锦鲤（颜色鲜艳，可烹饪",
    "Quadruples your": "四倍你的",
    "Requires coal to be present under the fire": "要求火下有煤",
    ...
    "Shovel some coal from the big pile they've prepared in the main square (Used to ignite fires": "从他们在主广场准备的大堆煤中铲出一些煤（用来生火",
    "Since you already own both, this has no additional value to you": "由于您已经拥有两者，因此这对您没有额外价值",
    "Some kids are selling a homemade slingshot. It looks pretty decent and is probably effective at shooting small objects to some height. You can't really think of a use for it at the moment though. Perhaps you'll find a use for it at a later time?": "一些孩子在卖自制的弹弓。 它看起来相当不错，可能可以有效地将小物体射到一定高度。 不过，您现在真的想不出它的用途。 也许您以后会发现它的用途？",
    "Spend five wooden chips to buy a packed meal (+2.80e6 health when eaten), contains all major food groups!": "花五个木片买一份盒饭（食用时 +2.80e6 生命值），包含所有主要食物组！",
    "Spend five wooden chips to buy a packed meal (": "花五个木片买一份盒饭（",
    "), contains all major food groups!": "），包含所有主要食物组！",
    "Spend two wooden chips to buy a mixed salad (+1.68e6 health when eaten), filled with five types of fruit and lettuce!": "花两块木片买一份混合沙拉（食用时 +1.68e6 生命值），里面有五种水果和生菜！",
    "Spend two wooden chips to buy a mixed salad (": "花两块木片买一份混合沙拉（",
    "), filled with five types of fruit and lettuce!": "），里面有五种水果和生菜！",
    "The blacksmith's dog is playing with some sort of stick. It isn't supposed to, but the blacksmith didn't really seem to have the time to take it away from the dog": "铁匠的狗正在玩某种棍子。 它不应该，但铁匠似乎真的没有时间把它从狗身上拿走",
    "The crowd is really dense now that the trade fair has started. You're welcome to join, but can't move much": "交易会已经开始，人群真的很密集。 欢迎加入，但不能多动",
    "The forager has a rather suboptimal way to teach you which mushrooms are edible. Just taste the mushrooms until you figure out which ones aren't toxic. This might have some unfortunate consequences on your stomach in the meantime": "采集者有一种不太理想的方法来教你哪些蘑菇可以食用。 尝尝蘑菇，直到你弄清楚哪些是无毒的。 同时，这可能会对您的胃造成一些不幸的后果",
    "The forager looks excited to meet someone that's interested in the mushroom collection!": "采集者见到对蘑菇收藏感兴趣的人，看起来很兴奋！",
    ...
    "These people seem to live inside the mountain itself. As if they used the mountain as the foundation of their city and dug through it": "这些人似乎就住在山里。 就好像他们以山为城基，将其挖通一样",
    "They appear to be staring at you. Who can blame them though? You've just arrived on the back of a dragon": "他们似乎在盯着你看。 谁能责怪他们呢？ 你刚刚骑在龙背上",
    "This cliff looks like it should be a relatively safe landing zone. But hold on tight. Hanging on to the dragon as it tries to land may be a little tricky": "这个悬崖看起来应该是一个比较安全的着陆区。 但要抓紧。 在龙试图着陆时抓住它可能有点棘手",
    "To keep competition fair, you're not allowed to resell this food": "为了保持公平竞争，您不得转售该食品",
    "Useful for consuming food. The blueprint says it should be closer to 5 steel, but getting the tines right proves tricky without further knowledge. Luckily nobody seems to pay enough attention to judge you": "对食用食物很有用。 蓝图说它应该更接近 5 钢，但在没有进一步了解的情况下，正确设置尖齿证明是棘手的。 幸运的是，似乎没有人足够重视来评判你",
    "Walk past the shacks to a waterfall at the end of the lower layer of the city. It appears to be their source of drinking water within the mountain": "走过窝棚，来到下层城市尽头的一处瀑布。 这似乎是他们在山中的饮用水源",
    "When you're all done trading, you can exit the square to return to the entrance of the city. You'll gain access to local food sources again by leaving the crowd too": "全部交易完后，就可以退出广场回到城门口。 你也可以离开人群，再次获得当地的食物来源",
    "When you're done exploring the top layer, walk back down. You do lose access to steel when you do": "探索完顶层后，向下走。 当你这样做时，你确实无法获得钢铁",
    "You already own cocowood soles, so you have no use for this debris": "你已经拥有椰子木鞋底，所以你不需要这些碎片",
    "After quite a few stairs, you've made it to the lower layer of the city within the mountain. It's notably humid in this area, and seems to house a fair amount of people. Some of the little shacks down here seem welcoming, while others are clearly closed": "走了好几级台阶，就来到了山内城的下层。 这个地区特别潮湿，似乎住着相当多的人。 这里的一些小棚屋似乎很受欢迎，而另一些则显然是封闭的",
    "To no surprise, you're welcomed by the local forager upon entering the shack! The shack is littered with piles of mushrooms, but you can't say you have a clue about whether or not they are edible": "毫不奇怪，您一进入小屋就会受到当地觅食者的欢迎！ 小屋里散落着成堆的蘑菇，但你不能说你知道它们是否可以食用",
    "You barely got a word in, the forager just kept on rambling about mushrooms. Long story short: it appears that this city within the mountain is covered with mushrooms, if you know where to look. And some of them are edible, but telling them apart takes some skill. The forager is happy to teach you!": "你几乎没插话，采集者只是不停地谈论蘑菇。 长话短说：如果您知道去哪里看的话，这座山中城市似乎长满了蘑菇。 其中一些是可食用的，但区分它们需要一些技巧。 觅食者很乐意教你！",
    "You've left the forager's shack. Let's find out what else is around in the lower part of the city!": "你已经离开了采集者的小屋。 让我们看看这座城市的下半部分还有什么！",
	...
	"There seems to be someone on top of the waterfall, removing debris from their water supply. That person appears to be making gestures at you, pointing at some things on top of the waterfall as well as downwards, and waving at you": "似乎有人在瀑布顶部，清除供水系统中的碎屑。 那个人似乎在对你做手势，指着瀑布上方和下方的一些东西，并向你挥手",
	...
	"Somehow climbing up always feels about twice as hard as climbing down. Anyhow, let's see what else is in this mountain city!": "不知怎的，爬上去总感觉比爬下难两倍。 总之，看看这座山城里还有什么！",
	"Coming down! A set of cocowood soles that increase the multiplier of your  agility skill by 25%! You catch it, you keep it!": "下来！ 一套椰子木鞋底，让你的敏捷技能倍数提高25%！ 你抓住它，你保留它！",
	...
	"It appears as if they are preparing a trade fair in the square. You hear some people talking about Prophecy Day. Has this something to do with your entrance? Either way, let's see if you can help out!": "看样子他们正在广场上准备一个交易会。 你听到一些人谈论预言日。 这跟你的入口有关系吗？ 无论哪种方式，让我们看看您是否可以提供帮助！",
	...
	"It started a little smokey, but eased into a beautiful fire in no time! The festivities are ready to begin! The square is getting really busy really quickly. You hear some people talking about the trade fair, as if they do this every other week. Maybe they were just looking for an excuse to throw another fair": "它开始有点冒烟，但很快就变成了美丽的火焰！ 庆祝活动已准备就绪！ 广场很快变得非常繁忙。 您会听到一些人谈论交易会，就好像他们每隔一周就谈论一次。 也许他们只是在找借口举办另一场博览会",
	"Well this crowd is packed for sure. You can't move much, but your inventory attracts some attention. It appears that they use some form of wooden chips as a currency for trade during the fair. Let's see if you can acquire something of interest!": "好吧，这群人肯定挤满了人。 你不能移动太多，但你的存货会引起一些注意。 看来他们在展会期间使用某种形式的木片作为交易货币。 让我们看看您是否可以获得感兴趣的东西！",
    "Provides 1 edible mushroom per completion (": "每次完成提供 1 个可食用蘑菇（",
	...
	"Since you don't own both of these tools yet, this may prove useful to you!": "由于您还没有这两种工具，因此这可能对您有用！",
	"Well that was a nice trade fair! You feel comfortable with the trades you've been able to make. Let's see what else is around in the city!": "那是一个不错的交易会！ 您对自己能够进行的交易感到满意。 让我们看看城里还有什么！",
	"The stairs were in a pretty good state, so it wasn't too much of a climb up. Now that you're here, you see that the smoke is coming from some furnaces. It seems that they manage their production of resources here, so that the smoke doesn't cover too much of the city": "楼梯状况很好，所以爬上去并不费力。 既然你在这里，你会看到烟雾从一些熔炉中冒出来。 看来他们在这里管理他们的资源生产，所以烟雾不会覆盖太多城市",
	"The blacksmith didn't seem too interested in a long conversation, but gave you some quick pointers. There are some construction blueprints on the table that you're free to use... Oh and the dog is playing with a stick that it shouldn't be playing with": "铁匠似乎对长时间的谈话不太感兴趣，但给了你一些快速的指示。 桌子上有一些建筑蓝图，你可以随意使用......哦，狗正在玩它不应该玩的棍子",
	"Shovel molten steel right out of the furnace! There must be a safer way to obtain this...": "直接从炉子里铲出钢水！ 必须有一个更安全的方法来获得这个......",
	"Well, the dog does not seem obedient whatsoever... If you're going to take its stick, you might get bitten in the process. Now that you've gotten a closer look, you see that it's holding an iron stick. That doesn't seem like a proper toy for a dog": "好吧，这只狗似乎一点也不听话……如果你要接过它的棍子，你可能会在这个过程中被咬伤。 现在仔细一看，原来是拿着一根铁棍。 这似乎不是适合狗的玩具",
	"Phew, while that smoke wasn't nearly as bad as you managed to survive in the volcano, it's still pretty nice to take a breath of cleaner air after walking down! Let's see what else is around town": "呼，虽然那烟远没有你在火山中幸存下来那么糟糕，但走下去后呼吸一口更清新的空气还是很不错的！ 让我们看看城里还有什么",
	...
	"However, you already own an iron stick, so you have no use for it": "不过你已经有铁棍了，没用",
	"Well, you managed to take the stick from the blacksmith's dog! You can't help but feel that this dog could use some behavioral training though. Now that you did manage to take its toy, it's almost instantly distracted by some birds and starts charging after those. You have a gut feeling that following it might be a good idea": "好吧，你设法从铁匠的狗那里拿走了棍子！ 你不禁觉得这只狗可以使用一些行为训练。 现在你确实设法拿走了它的玩具，它几乎立即被一些鸟分散了注意力并开始追赶那些鸟。 你有一种直觉，遵循它可能是个好主意",
	"As you approach the vault, you can't help but feel impressed by just how enormous this door is. What could possibly have needed a door this big? You notice that some locals in ceremonial robes are awaiting you. It looks like they've been preparing for this day. But the well-worn state of their robes suggests that time has caused preparations to become more of an old tradition rather than anything else. Yet their faces indicate something between solemnity and an overwhelming surprise they are trying to mask": "当您走近金库时，您不禁会对这扇门的巨大印象深刻。 有什么可能需要这么大的门？ 您注意到一些身着礼仪长袍的当地人正在等您。 看来他们一直在为这一天做准备。 但他们长袍的破旧状态表明，时间已经使准备工作变得更像是一种古老的传统，而不是其他任何东西。 然而，他们的脸上表现出介于严肃和他们试图掩饰的压倒性惊讶之间的某种东西",
	"The dog charged after some birds right after you took its toy. Let's see where it leads you!": "在您拿走它的玩具后，这只狗立即冲向一些鸟儿。 让我们看看它会把你引向何方！",
	"The dog led you to a nest of crows! They all fled as soon as the dog approached. You see some glimmering things in their nest": "狗把你带到了乌鸦的巢穴！ 狗一靠近，他们就全都逃走了。 你在它们的巢穴中看到一些闪闪发光的东西",
	...
	"The keepers welcome you, and get right to it. \"We've been guarding this vault for generations, without even being sure if anyone would ever prove worthy of reopening it. Legend has it that if anyone worthy ever arrived, it would be in a manner that others would consider impossible. So you understand our surprise when you flew in on the back of a greenscale. It isn't for us to judge though. The vault was created by the Architect himself, and should be able to determine for itself\"": "守门员欢迎你，并做好准备。 “我们几代人以来一直守护着这个金库，甚至不确定是否有人会证明值得重新打开它。传说如果任何有价值的人曾经到达，那将是其他人认为不可能的方式。所以你明白 当你骑着绿色鳞片飞进来时我们很惊讶。不过这不是我们可以判断的。这个拱顶是建筑师自己创造的，应该可以自己判断。”",
	"Now that the crows fled from the dog, let's grab everything valuable from their nest!": "既然乌鸦逃了狗，我们就把他们窝里值钱的东西抢过来！",
	"Provides a full inventory of steel on completion. This reward can surpass your inventory capacity if you already hold steel": "完成后提供完整的钢材库存。 如果你已经持有钢铁，这个奖励可以超过你的库存容量",
	"So many shiny things in such a small nest! But uh oh, the dog seems to have lost interest, and the crows are returning to their nest!": "这么小的窝里有这么多闪闪发光的东西！ 但是，呃，狗似乎失去了兴趣，乌鸦正在返回它们的巢穴！",
	"Ask the keepers who the Architect is": "询问管理员谁是建筑师",
	"\"Ah of course, you're not from around here and couldn't know this. Our ancestors used to follow the Architect on his journey. He was believed to hold the knowledge of a hundred lifetimes. He parted ways with our ancestors after completing the construction of the vault, leaving us with the task of guarding it against entry of unworthy thiefs or demolishers\". You can't help but feel that this Architect may just be similar to Amy and you. But how would they remember, if he truly was? Let's see if you can open the vault!": "“啊，当然，你不是这附近的人，不可能知道这些。我们的祖先曾经跟随建筑师的旅程。他被认为拥有一百生世的知识。他在完成后与我们的祖先分道扬镳。 金库的建造，留给我们的任务是防止不值得的窃贼或破坏者进入”。 您不禁会觉得，这位 建筑师 可能只是类似于 艾米 和您。 但他们怎么会记得，如果他真的是？ 让我们看看您是否可以打开金库！",
	"These things want you out of their nest as soon as possible and are coming at you with their entire flock. Let's see if you can scare them away like the dog did": "这些东西希望你尽快离开它们的巢穴，并带着它们的整个羊群向你袭来。 看看你能不能像狗一样把他们吓跑",
	...
	"As you get closer to the center of the vault door, your hourglass starts to tremble. As if these things are somehow linked? Perhaps your hourglass is the key to opening this door": "当你靠近金库门的中央时，你的沙漏开始颤抖。 好像这些东西有某种联系？ 也许你的沙漏是打开这扇门的钥匙",
	...
	"It worked! Channeling your hourglass in front of the door started lighting up symbols engraved in the door initially, until it eventually started making loud noises before opening slightly. It should be far enough open for you to enter now! Let's see if the keepers have any final advice for you": "有效！ 将你的沙漏引导到门前，最初会点亮刻在门上的符号，直到它最终开始发出响亮的声音，然后才稍微打开。 现在应该足够开放让你进入了！ 让我们看看饲养员是否有任何最终建议给你",
	"Let's see if they have any final advice before you enter the vault!": "在您进入金库之前，让我们看看他们是否有任何最终建议！",
	"They weren't as talkative as before, clearly overwhelmed by the moment that multiple generations of their families had been waiting for. \"We don't know what awaits you inside. But you can dispose of any leftover resources here. You've proven worthy by the vault itself. I'm sure you'll manage whatever the Architect guarded inside!\"": "他们不像以前那么健谈了，显然被他们几代人的家人一直在等待的那一刻压得喘不过气来。 “我们不知道里面等着你的是什么。但你可以在这里处理任何剩余的资源。你已经被保险库本身证明是有价值的。我相信你会管理建筑师在里面守卫的一切！”",
	"Enter the Architect's vault. By entering the vault, you'll lose access to food sources in the city. You'll also leave construction resources behind to free up inventory space": "进入建筑师的金库。 进入保险库后，您将无法获得城市中的食物来源。 您还将留下建筑资源以释放库存空间",
	"Walking into the vault, you see a mostly empty room. It's enormous and even smells as if nobody has entered this room in decades. In the very center of the room there's one enormous humanoid-like stone shape. It seems like a statue, but you can't quite tell for sure": "走进金库，您会看到一个几乎空无一人的房间。 它很大，甚至闻起来好像几十年来没有人进入过这个房间。 在房间的正中央，有一个巨大的人形石形。 它看起来像一座雕像，但你不能确定",
	"There's an enormous humanoid-like stone shape in the middle of the vault. Let's find out what it is": "金库中央有一个巨大的类人石形。 让我们找出它是什么",
	"As you get somewhat close, it quickly becomes very clear that this isn't a real statue. The stone limbs start moving, making sounds as if as if the cave itself is moving. This thing clearly isn't what the vault is guarding. It's another layer of defense": "当你走近一些时，很快就会很清楚这不是一座真正的雕像。 石肢开始移动，发出声音，就好像洞穴本身在移动一样。 这东西显然不是金库看守的东西。 这是另一层防御",
	"You have entered the  Stone Titan boss fight.": "你进入了石泰坦Boss战。",
	"During boss fights, only milestones such as health thresholds or unique explorations advance the story, rather than every exploration": "在Boss战中，只有生命值阈值或独特探索等里程碑才会推进故事，而不是每次探索",
	"Stone Titan": "石泰坦",
	"combat type": "战斗类型",
	"dodge type": "闪避类型",
	"block type": "格挡类型",
	...
	"This thing is coming at you with big swings! But what doesn't hit you won't hurt you!": "这东西正大摇大摆地向你袭来！ 但是打不中你的东西就不会伤害你！",
	"After successfully dodging the titan three times, you see a chance to fire an arrow at its back!": "成功躲避泰坦 3 次后，你就有机会向它的背后射箭！",
	"After successfully dodging the titan three times, you see a chance to trip it, causing it to lose its balance! That might just result in something useful": "成功躲避泰坦 3 次后，你有机会绊倒它，让它失去平衡！ 这可能只会产生一些有用的东西",
	...
	"If this thing is going to hit you, you'd better be prepared!": "如果这东西要袭击你，你最好做好准备！",
	...
	"Cooks 1 gollum clam per completion (+1.03 M health when eaten), requires a gollum clam to cook": "每次完成烹饪 1 个咕噜蛤蜊（食用时 +1.03 M 健康），需要咕噜蛤蜊来做饭",
	"Cooks 1 gollum clam per completion (": "每次完成烹饪 1 个咕噜蛤蜊（",
	"), requires a gollum clam to cook": "），需要咕噜蛤蜊来做饭",
	...
	"At first glance, it looks as if this was left over by the creator of the Titan. Some resources are hard to indentify, but you should find some use for them! But wait, the titan has recollected itself and starts taking aim at you from a distance!": "乍一看，这似乎是泰坦的创造者留下来的。 有些资源很难识别，但您应该找到它们的用处！ 但是等等，泰坦已经回过神来，开始从远处瞄准你！",
	...
	"Return to the Titan to continue combat and stop its ranged barrage. You'll probably want to complete as many constructions as you can, before it's arrows become overwhelming": "返回泰坦继续战斗并停止其远程弹幕。 在箭矢变得势不可挡之前，您可能希望尽可能多地完成构造",
	...
	"Provides 1 portion of honey per completion (+1.26 M health when eaten": "每次完成提供 1 份蜂蜜（食用时 +1.26 M 生命值",
	"Provides 1 portion of honey per completion (": "每次完成提供 1 份蜂蜜（",
	...
	"Provides 1 portobello mushroom per completion (+1.43 M health when eaten": "每次完成提供 1 个双孢蘑菇（食用时 +1.43 M 生命值",
	"Provides 1 portobello mushroom per completion (": "每次完成提供 1 个双孢蘑菇（",
	...
	"This thing is like nothing you've seen before, it's a combination of lights and cogs that seems to function as some sort of self-powering energy source. At this point, your hourglass has started to tremble in a way you've only seen a couple of times before. You have a strong feeling that your hourglass can funnel some of the power from this core. Yet feel inclined to disable it before moving on. Even if you're not fully sure why the inscription warned for this source of power, you feel inclined to oblige": "这东西和你以前见过的任何东西都不一样，它是灯和齿轮的组合，似乎起到某种自供电能源的作用。 此时，您的沙漏开始以您以前只见过几次的方式颤抖。 你有一种强烈的感觉，你的沙漏可以从这个核心中汲取一些力量。 然而，在继续之前倾向于禁用它。 即使您不完全确定为什么铭文会警告这种力量来源，您还是觉得有义务这样做",
	...
	"Disabling the core will create a fiery shockwave that will blast away most of what's left of the vault, any leftover resources in your carts, and will quadruple your  health decay": "禁用核心将产生一个炽热的冲击波，它会炸毁金库中剩下的大部分东西，以及您购物车中的任何剩余资源，并且会使您的  生命值衰退翻了两番",
    ...
    "Provides 1 corncob per completion (": "每次完成提供 1 根玉米（",
    "Provides 1 berry per completion (+5.95 health when eaten": "每次完成提供 1 个浆果（食用时 +5.95 生命值",
    "Provides 1 berry per completion (": "每次完成提供 1 个浆果（",
...
var cnPrefix = {
    ...
    "Funnel shield completions: ": "漏斗护盾完成：",
...
//    [/^(.+) health when eaten$/, '食用时恢复 $1 生命值'],
    ...
    [/\+([\d\.]+) ([K|M|B|T|Qa]) health when eaten/, '食用时 +$1 $2 生命值'],
    [/\+([\d\.]+) health when eaten/, '食用时 +$1 生命值'],
    [/\+([\d\.]+)e([\d\.,]+) health when drunk/, '喝下时 +$1e$2 生命值'],
    [/\+([\d\.]+) ([K|M|B|T|Qa]) health when drunk/, '喝下时 +$1 $2 生命值'],
    [/\+([\d\.]+) health when drunk/, '喝下时 +$1 生命值'],
    [/^([\d\.]+)e([\d\.,]+) elves$/, '$1e$2 精灵'],
