...
//    [/^(.+) health when eaten$/, '食用时恢复 $1 生命值'],
	...
	[/^These beasts may look harmless, but they definitely aren't. Every completion causes ([\d\.]+(?:(?: [K|M|B|T|Qa])|(?:e(?:[\d\.,]+)))?) damage, and provides 1 raw seal per completion \(Can be cooked/, "这些野兽可能看起来无害，但绝对不是。 每次完成都会造成 $1 伤害，并且每次完成提供 1 个生印章（可以煮熟"],
	[/^Very hot. Every completion causes ([\d\.]+(?:(?: [K|M|B|T|Qa])|(?:e(?:[\d\.,]+)))?) damage, and provides 1 magma block per completion \(Used for construction/, "很烫。每次完成造成 $1 伤害，并且每次完成提供 1 个熔岩块（用于建造"],
	[/Every completion causes ([\d\.]+(?:(?: [K|M|B|T|Qa])|(?:e(?:[\d\.,]+)))?) damage, and provides 1 steel per completion \(Used for construction/, "每次完成造成 $1 伤害，并且每次完成提供 1 钢（用于建造"],
