...
var cnItems = {
    ...
    "level up slower, but persist through death, so they'll improve how well you'll perform at actions that use that skill in all following lives. The combination of these levels determines the speed at which you can perform actions that use this skill, as well as affect the experience per second for both.": "升级速度较慢，但会在死亡中坚持下去，因此它们会提高您在接下来的所有生活中使用该技能的动作的表现。这些级别的组合决定了您可以使用此技能执行操作的速度，并影响两者的每秒体验。",
    "License": "执照",
    ...
    "Cooks 1 koi carp per completion (+588 K health when eaten), requires a raw koi carp to cook": "每完成一次烹饪1条锦鲤(食用时增加500K生命值)，需要一条生锦鲤来烹饪",
    "Farming skill multiplier of 5.00 B to succeed, and fails if your multiplier is lower": "耕作技能乘数为5.00 B才能成功，如果乘数较低则失败",
	...
	"As you get somewhat close, it quickly becomes very clear that this isn't a real statue. The stone limbs start moving, making sounds as if as if the cave itself is moving. This thing clearly isn't what the vault is guarding. It's another layer of defense": "当你走近一点，很快就会发现这不是一座真正的雕像。石头的四肢开始移动，发出声音就好像洞穴本身也在移动。这东西显然不是金库守护的东西。这是另一层防御。",
	...
	"Cooks 1 koi carp per completion (+654 K health when eaten), requires a raw koi carp to cook": "每次完成烹饪1条锦鲤(食用时增加654 K生命值)，需要一条生锦鲤来烹饪",
	"digging and": "挖掘和",
	"Digging skill. Resources required stay the same, but aren't all required at once  |  250 B exp": "挖掘技巧。所需的资源保持不变，但不是一次都需要| 250 B exp",
	...
	"During boss fights, only milestones such as health thresholds or unique explorations advance the story, rather than every exploration": "在boss战斗中，只有生命值阈值或独特的探索等里程碑才能推动故事发展，而不是每一次探索。",
	"Every completion causes 500 K damage, and provides 1 steel per completion (Used for construction": "每次完井造成500K损伤，每次完井提供1钢(用于施工)",
	"Find the update notes on Steam": "在Steam上找到更新说明",
	"Game update detected!": "检测到游戏更新!",
	"Game version: 21 January 2023": "游戏版本:2023年1月21日",
	...
	"Hunting skill multiplier of 7.50 B to succeed, and fails if your multiplier is lower": "狩猎技能乘数为7.50B才能成功，如果乘数较低则失败",
	...
	"Well this crowd is packed for sure. You can't move much, but your inventory attracts some attention. It appears that they use some form of wooden chips as a currency for trade during the fair. Let's see if you can acquire something of interest!": "这里肯定挤满了人。你不能移动太多，但你的库存吸引了一些注意力。他们似乎在集市上使用某种形式的木屑作为交易货币。让我们看看你是否能得到一些有趣的东西!",
